...
(async () => {
  ...
  //  このループが保証するのは「各期間ボタンを押したとき、副題がその期間**専用**の文言に
  //  なっていて、他の期間の文言と取り違えていないこと」だけ（正規表現を緩めて「どれでも
  //  緑」にしない、という意味で期間ごとに別の正規表現を割り当てている）。
  //  ⚠ D2（ローリング窓を選んだ状態で年度ボタン＝switchYear を押すと期間バーが FY に
  //  戻る、という2コントロール間の状態調停）はここでは検知できない。このループは
  //  #w2-period-box の期間ボタンしか押しておらず、.time-btn（年度ボタン）を一度も
  //  押さないため。D2 の回帰検知は scratchpad/w2-smoke.js（8220番・W2_INJECT=0）の
  //  「FY ボタンを押すと期間バーが FY に戻る」ケースの責務＝wave-closure.sh（8200番）
  //  だけを回しても D2 は見えない（前 wave 単発の「7203.T の FY 副題」アサートは
  //  このループに吸収したが、意味的には別物として扱うこと）。
  const EXPECT = {
    FY: /^\[.*経営期間トレンド\]$/,
